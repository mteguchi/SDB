...
dlines <- 100    # distance between lines in m
...
```{r}
# N most latitude for transect lines
N.most.lat <- 32.660
S.most.lat <- 32.604

# approximate center of the study area
center.latlon <- data.frame(long = -117.128, lat = 32.6411)

# create a line along this latitude (longitude values are arbitrary: wide enough to cover SDB):
N.most.line <- SpatialLines(list(Lines(list(Line(data.frame(long = c(-118, -116),
...
while (lat > S.most.lat){
  ...
  lat <- tmp.pt$lat
  new.line <- SpatialLines(list(Lines(list(Line(data.frame(long = c(-118, -116),
                                                           lat = c(lat, lat)))),
                                      ID = "a")),
                           proj4string = CRS("+proj=longlat +datum=WGS84"))
...
# in Google Earth to plot end points easily
